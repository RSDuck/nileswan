...
void mcu_spi_set_freq(uint32_t freq) {
    LL_GPIO_SetPinSpeed(NILE_PORT_SPI, NILE_PIN_MASK_SPI_SCK, freq);
    LL_GPIO_SetPinSpeed(NILE_PORT_SPI, NILE_PIN_MASK_SPI_POCI, freq);
    LL_GPIO_SetPinSpeed(NILE_PORT_SPI, NILE_PIN_MASK_SPI_PICO, freq);
}
...
void mcu_init(void) {
    ...
    for (uint32_t pin = LL_GPIO_PIN_4; pin <= LL_GPIO_PIN_7; pin <<= 1) {
        LL_GPIO_SetAFPin_0_7(NILE_PORT_SPI, pin, LL_GPIO_AF_0);
        LL_GPIO_SetPinOutputType(NILE_PORT_SPI, pin, LL_GPIO_OUTPUT_PUSHPULL);
        LL_GPIO_SetPinPull(NILE_PORT_SPI, pin, LL_GPIO_PULL_NO);
        LL_GPIO_SetPinMode(NILE_PORT_SPI, pin, LL_GPIO_MODE_ALTERNATE);
    }
    LL_GPIO_SetPinSpeed(NILE_PORT_SPI, NILE_PIN_MASK_SPI_NSS, LL_GPIO_SPEED_FREQ_LOW);
    mcu_spi_set_freq(NILE_SPI_FREQ_384KHZ);

    LL_SPI_Disable(NILE_PERIPH_SPI);
    LL_SPI_SetMode(NILE_PERIPH_SPI, LL_SPI_MODE_SLAVE);
    LL_SPI_SetTransferDirection(NILE_PERIPH_SPI, LL_SPI_FULL_DUPLEX);
    LL_SPI_SetClockPolarity(NILE_PERIPH_SPI, LL_SPI_POLARITY_LOW);
    LL_SPI_SetClockPhase(NILE_PERIPH_SPI, LL_SPI_PHASE_1EDGE);
    LL_SPI_SetBaudRatePrescaler(NILE_PERIPH_SPI, LL_SPI_BAUDRATEPRESCALER_DIV2);
    LL_SPI_SetDataWidth(NILE_PERIPH_SPI, LL_SPI_DATAWIDTH_8BIT);
    LL_SPI_SetNSSMode(NILE_PERIPH_SPI, LL_SPI_NSS_HARD_INPUT);

    // Initialize FPGA IRQ pin
    LL_GPIO_SetPinOutputType(GPIOB, NILE_PIN_MASK_FPGA_IRQ, LL_GPIO_OUTPUT_PUSHPULL);
    LL_GPIO_SetPinPull(GPIOB, NILE_PIN_MASK_FPGA_IRQ, LL_GPIO_PULL_NO);
    LL_GPIO_SetPinSpeed(GPIOB, NILE_PIN_MASK_FPGA_IRQ, LL_GPIO_SPEED_FREQ_LOW); // 400 kHz
    LL_GPIO_SetOutputPin(GPIOB, NILE_PIN_MASK_FPGA_IRQ);
    LL_GPIO_SetPinMode(GPIOB, NILE_PIN_MASK_FPGA_IRQ, LL_GPIO_MODE_OUTPUT);

    // Initialize VBUS sensing
    LL_GPIO_SetPinPull(GPIOB, NILE_PIN_MASK_USB_POWER, LL_GPIO_PULL_DOWN);
    LL_GPIO_SetPinSpeed(GPIOB, NILE_PIN_MASK_USB_POWER, LL_GPIO_SPEED_FREQ_LOW);
    LL_GPIO_SetPinMode(GPIOB, NILE_PIN_MASK_USB_POWER, LL_GPIO_MODE_INPUT);
